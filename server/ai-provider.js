...
async function callRemote(messages, maxTokens = 1800) {
  ...
  try {
    ...
    let data; try { data = JSON.parse(raw); } catch { data = { error: { message: raw.slice(0, 500) } }; }
...
function buildMessages({ prompt, memoryText, context, mode }) {
  const system = [
    'Tu es le cerveau d’Et Si? AI, une IA conversationnelle spécialisée dans le raisonnement et les réalités alternatives.',
    'Comprends la question avant de répondre. Ne répète pas mécaniquement les mêmes phrases.',
    'Utilise la mémoire comme un état du monde, pas comme du texte à recopier.',
    'Si la question change de sujet, traite-la comme une nouvelle question sans mélanger les scénarios.',
    'Pour une réalité alternative, respecte les hypothèses, déduis les conséquences et conserve une chronologie cohérente.',
    'Ne prétends jamais avoir fait une recherche que tu n’as pas faite.',
    'Réponds naturellement en français sauf demande contraire.',
    `Mode : ${mode || 'Long'}.`
  ].join('\n');
  const user = [
    `QUESTION:\n${cap(prompt, MAX_AI_PROMPT)}`,
    `MEMOIRE:\n${cap(memoryText, MAX_AI_MEMORY) || 'Aucune mémoire : nouvelle conversation ou nouveau scénario.'}`,
    `CONTEXTE:\n${cap(context, MAX_AI_CONTEXT) || 'Aucun contexte externe.'}`,
    'Réponds maintenant. Ne recommence pas inutilement une histoire déjà commencée et réponds précisément à la dernière question.'
  ].join('\n\n');
...
export async function generateWithAI({ prompt, memoryText, context, mode }) {
  ...
  if (aiConfigured) {
    try {
      ...
      console.error('Remote AI failed; using local model:', error?.message || error);
    }
  }
  return generateLocal(messages, mode === 'Long' ? 850 : 500);
}
